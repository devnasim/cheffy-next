...
function Button({ type, name, secondary, ...props }) {
  if (secondary) {
    return (
      <button
        className="font-sans bg-white py-2 px-6 text-pink-600 font-semibold rounded-lg shadow-md hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-pink-600"
        type={type}
        {...props}
      >
        {name}
